...
def cleanup(os_cloud, days=0):
    ...
    def _remove_volumes_from_cloud(volumes, cloud):
        print(
            "Removing {} volumes from {}.".format(len(volumes), cloud.cloud_config.name)
        )
        for volume in volumes:
            try:
            ...
            if not result:
                ...
                print(
                    'Removed "{}" from {}.'.format(volume.name, cloud.cloud_config.name)
                )
...
def remove(os_cloud, volume_id, minutes=0):
    ...
    if datetime.strptime(
        ...
        print(
            'WARN: volume "{}" is not older than {} minutes.'.format(
                volume.name, minutes
            )
        )
    else:
